...
const ITEM_H = 48;
const COL_H = ITEM_H * 5;

function DrumColumn({ items, selected, onSelect, width = 80 }: {
  ...
  return (
    <div
      ref={ref}
      style={{ height: COL_H, width, overflowY: 'scroll', scrollSnapType: 'y mandatory', scrollbarWidth: 'none', position: 'relative', zIndex: 1 }}
    ...
    >
      ...
      {items.map((item) => (
        ...
        >
          <span
            className="font-['Roboto',sans-serif] font-medium transition-all duration-150 select-none"
            style={{ fontSize: item === selected ? 26 : 20, color: item === selected ? '#000000' : '#c9cdd4', fontWeight: item === selected ? 600 : 400 }}
          >
            {item}
          </span>
        </div>
...
function TimePickerOverlay({ value, onChange, onClose }: {
  value: string; onChange: (t: string) => void; onClose: () => void;
}) {
  const parse = (t: string) => {
    const m = t.match(/(\d+):(\d+)\s*(AM|PM)/i);
    return m ? { h: m[1], min: m[2], per: m[3].toUpperCase() } : { h: '8', min: '00', per: 'AM' };
  };
  const init = parse(value);
  const [hour, setHour] = useState(init.h);
  const [minute, setMinute] = useState(init.min);
  const [period, setPeriod] = useState(init.per);
  ...
  return (
    <>
      ...
      <div className="absolute left-0 right-0 bottom-0 bg-white z-50 rounded-t-[16px]">
        ...
        {/* Drum */}
        <div className="relative flex items-center justify-center" style={{ height: COL_H }}>
          {/* Highlight band */}
          <div
            className="absolute left-[16px] right-[16px] bg-[#f4f4f5] rounded-[12px] pointer-events-none"
            style={{ top: ITEM_H * 2, height: ITEM_H }}
          />
          <DrumColumn items={hours} selected={hour} onSelect={setHour} width={90} />
          <span className="font-['Roboto',sans-serif] font-medium text-[26px] text-[#1a1a1a] z-10 leading-none pb-[2px]">:</span>
          <DrumColumn items={minutes} selected={minute} onSelect={setMinute} width={90} />
          <div style={{ width: 20 }} />
          <DrumColumn items={['AM', 'PM']} selected={period} onSelect={setPeriod} width={70} />
        </div>

        {/* Buttons */}
        <div className="flex gap-[12px] px-[16px] py-[16px]">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 h-[48px] border border-[#dadce0] rounded-full font-['Roboto',sans-serif] font-medium text-[15px] text-[#373737]"
          >
            Cancelar
          </button>
          <button
            type="button"
            onClick={() => { onChange(`${hour}:${minute} ${period}`); onClose(); }}
            className="flex-1 h-[48px] bg-[#5c96f6] rounded-full font-['Roboto',sans-serif] font-medium text-[15px] text-white"
          >
            Confirmar
          </button>
        </div>
...
function FieldBox({ label, chevron = false, withAdd = false, withEdit = false, counter, tall = false, options, charLimit }: {
  label: string; chevron?: boolean; withAdd?: boolean; withEdit?: boolean; counter?: string; tall?: boolean; options?: string[]; charLimit?: number;
}) {
  ...
  const borderColor = isOverLimit ? '#ef4444' : '#dadce0';

  return (
    <div>
      <div className="flex items-start gap-[8px]">
        <div className="flex-1 rounded-[4px] px-[12px] pt-[6px]" style={{ minHeight: tall ? 72 : 48, border: `1px solid ${borderColor}` }}>
          ...
          {chevron ? (
            ...
            <textarea
              ...
              rows={2}
              onChange={charLimit != null ? (e) => setCharCount(e.target.value.length) : undefined}
            />
          ) : (
      ...
      {charLimit != null ? (
        <span className="font-['Roboto',sans-serif] text-[11px] mt-[2px] block" style={{ color: isOverLimit ? '#ef4444' : '#9ca3af' }}>
          {charLimit} disponibles {charCount} escritos{isOverLimit ? '. Excede caracteres disponibles.' : ''}
        </span>
      ) : counter ? (
...
function InfoBasicaForm({ onFechaClick, selectedDate, onHoraClick, selectedTime }: {
  onFechaClick: () => void; selectedDate: Date | null;
  onHoraClick: () => void; selectedTime: string;
}) {
  ...
  return (
    <div className="flex flex-col gap-[8px] p-[12px]">
      ...
      <FieldBox label="Sección" chevron options={['Nacionales','Internacionales','Deportes','Economía','Cultura','Tecnología']} />
      <FieldBox label="Subsección" chevron options={['Sub 1','Sub 2','Sub 3']} />
      <FieldBox label="Título*" tall />
      <FieldBox label="Título de SEO" tall charLimit={60} />
      <FieldBox label="Subtítulo*" tall />
      <FieldBox label="Subtítulo SEO *" tall charLimit={150} />
      ...
      {/* Hora — custom field opens time picker */}
      <div className="border border-[#dadce0] rounded-[4px] px-[12px] pt-[6px] pb-[2px]" style={{ minHeight: 48 }}>
        <label className="font-['Roboto',sans-serif] text-[11px] text-[#9ca3af] block">Hora*</label>
        <button type="button" onClick={onHoraClick} className="w-full flex items-center justify-between pb-[6px]">
          <span className="font-['Roboto',sans-serif] text-[14px] text-[#373737]">{selectedTime}</span>
          <ChevronDownIcon />
        </button>
      </div>
      <FieldBox label="Lugar *" chevron withAdd withEdit options={['Santo Domingo','Santiago','La Vega','Puerto Plata','San Pedro de Macorís']} />
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          {selectedEditor === "Info. Básica" ? (
            <InfoBasicaForm
              onFechaClick={() => setShowDatePicker(true)} selectedDate={selectedDate}
              onHoraClick={() => setShowTimePicker(true)} selectedTime={selectedTime}
            />
          ) : (
